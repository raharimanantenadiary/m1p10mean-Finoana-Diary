...
export class AcceuilComponent implements OnInit {
  ...
  verifierBS(idvoiture:any) {
     return this.service_financier.verifieBS(idvoiture).subscribe(response => {
      this.bon_check = response;
      localStorage.setItem("etat",this.bon_check.etat);
      // return this.bon_check;
      // console.log("test",this.bon_check);
    });
  }
  ...
  drop(event: CdkDragDrop<any[]>) {
    ...
    // console.log("etat", this.verifierBS(this.bon_check));
    if (event.previousContainer === event.container) {
      ...
      if (event.item.data.voiture.etat == 1) {
        ...
        this.service_financier.verifieBS(event.item.data.voiture._id).subscribe(response => {
          this.bon_check = response;
          console.log('Etat === 1',this.bon_check);
        });
        let i = this.bon_check;
        console.log('i',i);

        // let li = this.verifierBS(event.item.data.voiture._id);

        // console.log("checking",this.verifierBS(event.item.data.voiture._id));
        // if (this.bon_check == true) {
        //   transferArrayItem(
        //     event.previousContainer.data,
        //     event.container.data,
        //     event.previousIndex,
