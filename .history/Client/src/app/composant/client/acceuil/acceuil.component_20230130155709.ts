...
//  }
  ...
  verifierBS() {
  this.bon_check =  this.service_financier.verifieBS("63d79a557f6a607f88ade292").subscribe(response => {
    return true;
    // localStorage.setItem("etat", this.diary.etat);
  });
...
drop(event: CdkDragDrop<any[]>) {
  if (event.previousContainer === event.container) {
    ...
    if (event.item.data.voiture.etat == 1) {
          console.log("bon",this.verifierBS());
